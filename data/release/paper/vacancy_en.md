...
> **This is a survey, not a proof. Evidence of absence is limited and does not prove the absence of occupation.**
> **A vacancy bound is not a settlement permit.**

## Abstract

For each star in the solar neighbourhood we aggregate conditional non-detection evidence of the form "if this star were occupied by facilities of a declared technology band T, the existing survey set S would have detected a trace", and tabulate for the first time a star-by-star, band-conditional upper bound on occupation, P(occupied | D, T, π), as a function of the prior π (an upper-bound curve). The population is the Gaia EDR3 Catalogue of Nearby Stars (GCNS), 331,312 stars, plus 1,259 bright stars missing from EDR3: 332,571 stars in total[^pop]. The claimed bands are three radio bands — T-R1 (narrowband, EIRP ≥ 10¹³ W), T-R2 (≥ 10¹⁷ W), and T-R3 (Earth-level radar-type intermittent leakage; the self-test band) — with the transmission frequency declared uniform within the window [1.10, 3.45] GHz. Detection probabilities ε are imported from the 356,616 observation rows (288,315 stars) of Wlodarczyk-Sroka et al. (2020), whose EIRP_min table we reproduce row by row with the Price et al. (2020) sensitivity formula (median ratio 1.0000); observations within one modality are merged by marginalising over shared latent variables (frequency ν, illumination factor f_ill) — we prove as a theorem that the naive per-observation likelihood product over-estimates the detection probability and tilts toward "vacancy". Thresholds, the aggregation rule, and pass criteria were pre-registered on Zenodo before any aggregation was run (doi:10.5281/zenodo.22067884). Verification gates: G1 monotonicity, 6,777 checks, 0 violations; G2 two independent implementations, max|Δlog₁₀Λ| ≤ 4.9 × 10⁻¹⁰ (radio); G3 three anchors (the WS20 population limit 1/N is reproduced exactly at ≤50 pc with N = 1513; the Hephaistos I count fails the frozen criterion, so the waste-heat band T-W1 is demoted to the extension shelf; the Solar-System self-test passes). Stars carrying a vacancy bound number 1,554 (T-R1), 1,587 (T-R2), and 159 (T-R3); the remaining 330,984 stars (99.52%) are outside every radio field and are **undecidable**, which we report as a main result (a star-level version of haystack incompleteness). In the Earth-level band T-R3 every star has posterior ≈ prior (Λ ≥ 0.9985): the lowest technology band about which this atlas can speak is calibrated by Earth itself. Joining reachability (EMBARK) and settlement-resource layers (GCNS, known exoplanets) into a three-axis atlas, 815 stars are both flyby-reachable and carry a T-R1 bound (no composite score is formed). The stellar basis is Gaia DR3 in a two-stage design: at the Gaia DR4 release (2026-12-02) only the distance-dependent part of the ε ledger will be recomputed for v1.1.
...
Self-reported items: one omission of an approved addendum sentence at the re-submission of revision 2; a subtraction error in the BL download tally script (no download was missed); a shell-expansion loss in a ruling-log path string — none affecting any number (Appendix B).

## 5. Results

### 5.1 Overview of the ledger
Stars carrying a vacancy bound (ε determined) number **1,554** in T-R1, **1,587** in T-R2, and **159** in T-R3 (d ≤ 10.8 pc)[^s1]. Stars observed but insensitive in the band (ε = 0, Λ = 1 = no information) number 33 in T-R1 and 1,428 in T-R3. In T-R1, Λ ranges from 0.4266 (minimum: four rows, GBT L + GBT S + GBT S + Parkes 10-cm, i.e. two S-band pointings, one star) through 0.8000 (median: GBT S only) to 0.8596 (maximum: GBT L only), with a median posterior of 0.0080 at π = 10⁻². In T-R3, Λ = 0.9985–0.9998 and the posterior at π = 10⁻² is 0.0100. Λ is set by the coverage structure of the observation rows (in-window coverage of the receiver × f_pipe) and hardly by distance (only 33 stars straddle the sensitivity threshold in T-R1).
...
Three roles: the human (Yukie Maeda) — direction, gate rulings, all final decisions at branch points; chat Claude — staff officer, independent recomputation (the second G2 path), literature cross-checks; Claude Code (Fable 5) — execution. Operational record: 6 rulings (#1 band set A–F; #2 pre-registration (b) revisions; #3 deviation, criterion revision; #4 acceptance of failure and demotion; #5 seven referred items; #6 approval of the outline), 2 stops (G3(i)(b), G3(ii) — both correct, resumed by ruling), 1 deviation, 1 failed anchor, 3 self-reports. Error ledger r1 (E-1, E-2). This internal verification is not a substitute for human peer review.
