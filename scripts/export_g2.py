...
D = np.load(os.path.join(P1, "ledger_v0_detail.npz"))
insts, M = E.cover_matrix()
rows_by_star = {}
for r in R["rows"]:
    rows_by_star.setdefault(r["star"], []).append([r["inst"], r["eps_in_band"]["R1"], r["eps_in_band"]["R2"], r["theta_R3"]])
ids = S["id"]
radio = {sid: {"rows": rows_by_star[sid], "ledger_eps": {"R1": S["eps_R1"][k], "R2": S["eps_R2"][k], "R3": S["eps_R3"][k]}}
         for k, sid in enumerate(ids) if sid in rows_by_star}
...
mir = {"gamma_levels": list(E.GAMMA_LEVELS), "n_T": len(E.T_CENTERS),
       "stars": {ids[k]: {"surv_g0.1": D["surv_W1_g01"][k].astype(int).tolist(), "surv_g0.5": D["surv_W1_g05"][k].astype(int).tolist(),
                          "surv_g0.9": D["surv_W1_g09"][k].astype(int).tolist(),
                          "ledger_eps": {"0.1": S["eps_W1_g0.1"][k], "0.5": S["eps_W1_g0.5"][k], "0.9": S["eps_W1_g0.9"][k]}} for k in okW}}
out = {"schema": "vacancy-g2-export-v0", "eps_formula_version": L["eps_formula_version"],
       "formula": {"radio_R1R2": "1-eps = sum_c w_c prod_s (1 - eps_s * cover[inst_s][c])",
