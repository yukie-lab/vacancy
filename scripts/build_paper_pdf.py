...
def convert(md_path, en=False):
    ...
    author_line = next(l for l in lines[1:8] if l.startswith("**"))
    ...
    in_items = False
    # 目次
    body.append(r"\section*{" + ("Contents" if en else "目次") + "}")
    body.append(r"\begin{itemize}\setlength{\itemsep}{0pt}")
    for h in heads:
        ind = r"\hspace{1.2em}" if h.startswith("### ") else ""
        body.append(r"\item[] " + ind + tex_inline(re.sub(r"\[\^\w+\]", "", h.lstrip("#").strip()), en))
    ...
    return ("\\documentclass[11pt]{article}\n\\usepackage[margin=2.7cm]{geometry}\n\\usepackage{amsmath,amssymb}\n"
            ...
            "\\author{" + tex_inline(author_line.replace("**", ""), en) + r"\thanks{" + tex_inline(footmeta.split(":", 1)[1].strip() if ":" in footmeta else footmeta, en) + "}}\n"
            "\\date{}\n\\begin{document}\n\\maketitle\n" + quote_block + "\n" + tex_body + "\n\\end{document}\n")
