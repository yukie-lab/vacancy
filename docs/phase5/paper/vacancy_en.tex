...
\begin{document}
\maketitle
\begin{center}\textbf{This is a survey, not a proof. Evidence of absence is limited and does not prove the absence of occupation.}\end{center}
\begin{center}\textbf{A vacancy bound is not a settlement permit.}\end{center}
\section*{Contents}
...
\item[] 8. Limitations and outlook
...
\item[] References (under full cross-check by the staff officer --- 23-item checklist)
\end{itemize}
\section*{Abstract}
...
\noindent \S{}2 position; \S{}3 method; \S{}4 verification including the deviation section; \S{}5 results; \S{}6 the three-axis atlas; \S{}7 interpretation discipline; \S{}8 limitations and outlook; Appendices A--D.\par\smallskip
...
\noindent \textbf{Diversity of limit formulae}: the percentage limits of Price et al. (2020) \S{}5.3 (formula not stated), the 1/N of WS20, our exact binomial limit, and our star-level Bayesian bound are all \textbf{different quantities} (\S{}4.3, \S{}7).\par\smallskip
...
\noindent Self-reported items: one omission of an approved addendum sentence at the re-submission of revision 2; a subtraction error in the BL download tally script (no download was missed); a shell-expansion loss in a ruling-log path string --- none affecting any number (Appendix B).\par\smallskip
...
\noindent Every statement in this paper obeys four sentences. (1) \textbf{This is a survey, not a proof} --- evidence of absence is limited and does not prove the absence of occupation. (2) \textbf{A vacancy bound is not a settlement permit} --- no inference of absence of occupation, nor of the legitimacy of settlement, may be drawn from this product. (3) \textbf{The HWC cross-reference table is not a composition} --- no quantity multiplies habitable candidacy by a vacancy bound. (4) \textbf{1/N and the star-level posterior are different quantities} --- their numerical proximity (\S{}4.3(c)) is coincidental. In addition, omitting the conditional clause (band, survey set, \(\pi\)), presenting a single number or ranking, forming products or weighted sums across the three axes, tilting undecidable stars toward vacancy, and using this product to justify settlement, contact, or transmission are prohibited. The W1 information layer has no established anchor and is not used for vacancy claims.\par\smallskip

\section*{8. Limitations and outlook}
\begin{itemize}\setlength{\itemsep}{1pt}
...
\section*{References (under full cross-check by the staff officer --- 23-item checklist)}
