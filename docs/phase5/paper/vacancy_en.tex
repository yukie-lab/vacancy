...
\par\smallskip{\small Figure 2: receiver coverage of the declared window [1.10, 3.45] GHz (top: the 9 common intervals and notches) and the merged \(\Lambda\) for each observation-row combination present in the ledger (bottom: T-R1; bar colours follow the posterior colour rule at \(\pi\) = 10\(^{-2}\)). Values are the frozen radio\_obs\_v0.json and lambda\_ledger.json. Reproduce: python3 scripts/build\_paper\_figs.py}
\end{figure}
