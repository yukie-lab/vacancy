<!-- arXiv 版: scripts/build_arxiv_version.py で md 正本から機械生成(裁定 #13)。本文・数値は md 正本と同一(参考文献 author-year 化・図 PDF 参照のみ)。 -->
...
[FIG] figs/fig1_g3iii_dlambda.pdf | Figure 1: the G3(iii) Solar-System self-test as d–Λ. Three curves (marginalised; f_ill = 10⁻², the upper end; the extreme f_ill = f_pipe = 1) with the sensitivity boundaries (Parkes 5.24 pc, GBT 10.9 pc). Values are the frozen g3_iii.json. Reproduce: python3 scripts/build_paper_figs.py
...
[FIG] figs/fig3_atlas_overview.pdf | Figure 3: atlas overview (projection onto the Galactic plane, machine-generated from sim_display_v1). Undecidable stars in grey (thinned 1/8); stars with a T-R1 bound in the value-neutral continuous colour of the posterior at π = 10⁻²; EMBARK-reachable stars as outlines; the Sun as a star marker. Same colour discipline as the simulator. Reproduce: python3 scripts/build_paper_figs.py
...
[FIG] figs/fig4_pi_sweep.pdf | Figure 4: upper-bound curves P(occupied | D, T, π) = expit(logit π + ln Λ). Curves use the frozen-ledger Λ (min/median/max of T-R1 and T-R3); the dotted line is the prior P = π. Reproduce: python3 scripts/build_paper_figs.py
