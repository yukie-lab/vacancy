\documentclass{aa}
\usepackage{graphicx}
% \usepackage{txfonts}  % A&A 推奨。tectonic のフォントバンドルが t1xsl を配信せず(403)ローカルでは無効化。MMS 側コンパイルでは有効化可
\usepackage{array}
\usepackage{url}
\graphicspath{{figs/}}
\begin{document}
\title{A Vacancy Atlas of the Solar Neighbourhood: Star-by-Star, Technology-Band-Conditional Upper Bounds on Occupation from Heterogeneous Non-Detections}
\titlerunning{A vacancy atlas of the solar neighbourhood}
\authorrunning{Y. Maeda}
\author{Yukie Maeda\thanks{The roles of AI systems and the complete verification protocol are disclosed in Appendix B; this internal verification is not a substitute for human peer review.}}
\institute{Independent Researcher, Tokyo (ORCID: 0009-0005-3401-9230)}
\date{Received --; accepted --}
\abstract{To our knowledge no star-level ledger of the star-by-star, band-conditional upper bound on occupation exists.}{For each star in the solar neighbourhood we aggregate conditional non-detections ("facilities of band T at this star would have been detected by survey set S") into a star-by-star, band-conditional upper bound on occupation, P(occupied | D, T, \(\pi\)), as a function of the prior \(\pi\).}{The population is 332,571 stars within 100 pc (Gaia EDR3 GCNS basis). The claimed bands are three radio bands (T-R1: EIRP \(\geq\) 10\(^{13}\) W; T-R2: \(\geq\) 10\(^{17}\) W; T-R3: Earth-level radar-type intermittent leakage) within [1.10, 3.45] GHz. Detection probabilities are imported from the 356,616 observation rows of Wlodarczyk-Sroka et al. (2020); observations within one modality are merged by marginalising shared latent variables, and we prove that the naive likelihood product tilts toward "vacancy". Thresholds and pass criteria were pre-registered before aggregation (doi:10.5281/zenodo.22067884).}{Verification: G1 monotonicity, 6,777 checks, 0 violations; G2 independent implementations agree to 4.9 \(\times\) 10\(^{-10}\) dex; G3: the WS20 1/N limit is reproduced exactly at \(\leq\)50 pc (N = 1513), the Hephaistos I count fails (the waste-heat band is demoted), and the Solar-System self-test passes. Stars with a bound number 1,554 (T-R1), 1,587 (T-R2) and 159 (T-R3); 330,984 stars (99.52\%) are undecidable --- a main result. In T-R3 posterior \(\approx\) prior (\(\Lambda\) \(\geq\) 0.998): the lowest band the atlas can speak about is calibrated by Earth itself. Joined with reachability and settlement resources, 815 stars are flyby-reachable with a T-R1 bound. At the Gaia DR4 release (2026-12-02) only the distance-dependent part of the ledger is recomputed (v1.1). These bounds are survey quantities, not proofs of absence, and license no inference about settlement (\S{}7.1).}{}
\keywords{extraterrestrial intelligence -- astrobiology -- solar neighborhood -- catalogs -- surveys -- methods: statistical}
\maketitle
...
\subsection{Population and stellar basis}
...
\subsection{Declared technology bands}
...
\subsection{Factor structure of the detection probability \(\varepsilon\)}
...
\subsection{Merging rule (latent-variable marginalisation) and theorem}
...
\subsection{Aggregation and the \(\pi\) sweep}
...
\subsection{G1 --- monotonicity and Monte-Carlo stability}
...
\subsection{G2 --- two independent implementations}
...
\subsection{G3(i) --- WS20 / Price anchors}
...
\subsection{G3(ii) --- Hephaistos I count reproduction (fail)}
...
\subsection{Overview of the ledger}
\noindent Stars carrying a vacancy bound (\(\varepsilon\) determined) number \textbf{1,554} in T-R1, \textbf{1,587} in T-R2, and \textbf{159} in T-R3 (d \(\leq\) 10.8 pc)\footnote{\texttt{data/phase2/lambda\_ledger.json} (summary.status\_counts, lambda\_*).}. Stars observed but insensitive in the band (\(\varepsilon\) = 0, \(\Lambda\) = 1 = no information) number 33 in T-R1 and 1,428 in T-R3. In T-R1, \(\Lambda\) ranges from 0.4266 (minimum: four rows, GBT L + GBT S + GBT S + Parkes 10-cm, i.e. two S-band pointings, one star) through 0.8000 (median: GBT S only) to 0.8596 (maximum: GBT L only), with a median posterior of 0.0080 at \(\pi\) = 10\(^{-2}\). In T-R3, \(\Lambda\) = 0.9985--0.9998 (rounded to four digits; exact minimum 0.998458) and the posterior at \(\pi\) = 10\(^{-2}\) is 0.0100. \(\Lambda\) is set by the coverage structure of the observation rows (in-window coverage of the receiver \(\times\) f\_pipe) and hardly by distance (only 33 stars straddle the sensitivity threshold in T-R1; Figure 2).\par\smallskip

\begin{figure}\centering
\includegraphics[width=\hsize]{figs/fig2_coverage_lambda.pdf}
\caption{Receiver coverage of the declared window [1.10, 3.45] GHz (top: the 9 common intervals and notches) and the merged \(\Lambda\) for the six most frequent observation-row combinations present in the ledger (bottom: T-R1; 1,515 stars in total, the remaining 39 stars being rarer combinations of three or more rows; bar colours follow the posterior colour rule at \(\pi\) = 10\(^{-2}\)). Values are the frozen radio\_obs\_v0.json and lambda\_ledger.json. Reproduce: python3 scripts/build\_paper\_figs.py}
\end{figure}



\subsection{99.52\% undecidable --- as a main result}
\noindent Of the 332,571 stars, \textbf{330,984 (99.52\%)} lie outside the field of every WS20 pointing\footnote{\texttt{data/phase2/lambda\_ledger.json} (summary.status\_counts, lambda\_*).}. Among them, \textbf{1,530 stars} have observation files in the BL open archive but no published limit (C/X band, or L/S/10-cm not in Price et al. 2020) and are classified as "observed, unpublished", undecidable (d)\footnote{\texttt{docs/phase1/00-status.md} \S{}1.3.}. Undecidable stars are tilted neither toward vacancy nor toward occupation. This number shows, star by star, \textbf{how few of the stars within 100 pc existing radio SETI non-detections actually reach}; it is the star-level version of the haystack incompleteness of Wright et al. (2018) (Figure 3).\par\smallskip

\begin{figure*}\centering
\includegraphics[width=\textwidth]{figs/fig3_atlas_overview.pdf}
\caption{Atlas overview (projection onto the Galactic plane, machine-generated from sim\_display\_v1). Undecidable stars in grey (thinned 1/8); stars with a T-R1 bound in the value-neutral continuous colour of the posterior at \(\pi\) = 10\(^{-2}\); EMBARK-reachable stars as outlines; the Sun as a star marker. Same colour discipline as the simulator. Reproduce: python3 scripts/build\_paper\_figs.py}
\end{figure*}



\subsection{Upper-bound curves}
\noindent P(occupied | D, T, \(\pi\)) = expit(logit \(\pi\) + ln \(\Lambda\)) is monotonic in \(\pi\); for the radio bands with \(\Lambda\) \(\in\) [0.43, 0.86] the posterior is \(\approx\) \(\pi\) \(\times\) \(\Lambda\) for \(\pi\) \(\ll\) 1. Figure 4 (\(\pi\) sweep).\par\smallskip

\begin{figure}\centering
\includegraphics[width=\hsize]{figs/fig4_pi_sweep.pdf}
\caption{Upper-bound curves P(occupied | D, T, \(\pi\)) = expit(logit \(\pi\) + ln \(\Lambda\)). Curves use the frozen-ledger \(\Lambda\) (min/median/max of T-R1 and T-R3); the dotted line is the prior P = \(\pi\). Reproduce: python3 scripts/build\_paper\_figs.py}
\end{figure}
\noindent  The "vacancy" is a curve, not a single number.\par\smallskip

\subsection{Solar self-calibration}
...
\subsection{The W1 information layer (claim=false)}
...
\section{Three-axis atlas}

\subsection{Join rates}
...
\subsection{Cross counts (no composition)}
\begin{table*}\centering{\scriptsize\setlength{\tabcolsep}{2.5pt}
\begin{tabular}{>{\raggedright\arraybackslash}p{0.594\textwidth}>{\raggedright\arraybackslash}p{0.346\textwidth}}
Condition & Stars \\
\hline
...
\subsection{Settlement-resource slider}
...
\subsection{HWC cross-reference table (not a composition)}
...
All data, code, and the pre-registration are openly available at Zenodo: \url{doi:10.5281/zenodo.22081202} (data and code) and \url{doi:10.5281/zenodo.22067884} (pre-registration). An interactive simulator is available at \url{https://yukie-lab.github.io/vacancy-atlas/}.
...
\bibliography{vacancy_refs}
\appendix
\section{\(\varepsilon\) factor table, receiver coverage, and the common \(\nu\) grid}
...
\section{Method record and AI disclosure}
...
\section{Undecidable accounting (three axes)}
\begin{table*}\centering{\scriptsize\setlength{\tabcolsep}{2.5pt}
\begin{tabular}{>{\raggedright\arraybackslash}p{0.496\textwidth}>{\raggedright\arraybackslash}p{0.362\textwidth}>{\raggedright\arraybackslash}p{0.083\textwidth}}
